 ...
 * Description:
 */
public class DiscStrategy extends Strategy {

    public DiscStrategy(Long hatchId, Integer bayNo, Map<Integer, WorkMove> rowNoMoveMap, MoveData moveData, WorkingData workingData, StructureData structureData) {
    ...
    public String getCurWorkFlow(String curWorkFlow) {
        //使用父类的方法
        curWorkFlow = super.getCurWorkFlowByDefault(CWPDomain.DL_TYPE_DISC, curWorkFlow, bayNo, rowNoMoveMap, moveData);
    ...
    public WorkMove getFirstWorkMove(String curWorkFlow, int curTierNo, List<WorkMove> delayWorkMoveList) {
        ...
        for (Integer rowNo : rowNoSeqList) {
            ...
            if (workMove1 != null && workMove1.getWorkFlow().equals(curWorkFlow) && workMove1.getTierNo() == curTierNo && workMove1.getDlType().equals(CWPDomain.DL_TYPE_DISC)) {
                workMove = workMove1;
                break;
            }
        }
